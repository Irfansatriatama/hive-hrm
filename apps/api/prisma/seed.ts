...
// Hashed password for "Admin@1234" generated by Better Auth's scrypt implementation
...
async function main() {
  ...
  const demoUsers = [
    { email: 'superadmin@hive.id', role: UserRole.SUPER_ADMIN, name: 'System Administrator', empId: 'EMP001' },
    { email: 'hradmin@hive.id', role: UserRole.HR_ADMIN, name: 'Sari Dewi Lestari', empId: 'EMP002' },
    { email: 'manager@hive.id', role: UserRole.MANAGER, name: 'Budi Santoso', empId: 'EMP003' },
    { email: 'karyawan@hive.id', role: UserRole.EMPLOYEE, name: 'Rina Agustina', empId: 'EMP004' },
    { email: 'finance@hive.id', role: UserRole.FINANCE, name: 'Dani Firmansyah', empId: 'EMP018' },
  ];
    ...
    if (!isDemo) {
      const user = await prisma.user.create({
        data: {
          ...
          role: emp.id === 'EMP001' ? UserRole.SUPER_ADMIN : emp.id === 'EMP002' ? UserRole.HR_ADMIN : emp.id === 'EMP003' ? UserRole.MANAGER : UserRole.EMPLOYEE,
          emailVerified: true,
        },
      });
