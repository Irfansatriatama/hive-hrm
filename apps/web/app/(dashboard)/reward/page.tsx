...
export default function RewardPage() {
  ...
  const loadRewardContext = async () => {
    setLoading(true);
    try {
      const [balVal, catalogData, empsData, txsData] = await Promise.all([
      ...
      setTransactions(txsData.slice(0, 5)); // show latest 5
    } catch (err) {
  ...
  const handleSendAppreciation = async (e: React.FormEvent) => {
    ...
    if (!recipientId || !hashtag || points < 10 || points > 100 || !message.trim() || sending) {
      alert('Mohon lengkapi seluruh field dan pastikan poin berada di rentang 10 - 100');
      return;
    }
  ...
  const handleRedeem = async (item: any) => {
    if (balance < item.points) {
      alert(`Poin Anda tidak mencukupi untuk menukar item ini. Butuh: ${item.points} Pts.`);
      return;
    }

    if (!confirm(`Apakah Anda yakin ingin menukarkan ${item.points} poin Anda untuk "${item.name}"?`)) {
      return;
    }
    ...
    try {
      ...
      alert('Item berhasil diredeem! Silakan ambil hadiah Anda di HRD.');
      loadRewardContext();
    } catch (err: any) {
  ...
  const hashtags = [
    { tag: '#Collab', desc: 'Collaboration & Teamwork' },
    { tag: '#Integrity', desc: 'Honesty & Ethical Work' },
    { tag: '#Innovation', desc: 'Creative Problem Solving' },
    { tag: '#Professional', desc: 'Expertise & Ownership' },
    { tag: '#Respect', desc: 'Empathy & Inclusivity' },
  ];
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
      ...
      <div className="lg:col-span-8 space-y-6">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4 select-none">
          <div>
            <h1 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Katalog Penukaran Reward</h1>
            ...
            {catalog.map((item) => {
              ...
              return (
                <div key={item.id} className="bg-white border border-slate-200/60 rounded-xl p-4.5 shadow-sm hover:shadow-md transition flex flex-col justify-between select-none">
                  <div>
                    {/* Visual Card Image */}
                    <div className="h-28 w-full bg-slate-50 rounded-lg flex items-center justify-center text-slate-400 mb-3 border border-slate-100">
                      <Lucide.Gift className="w-8 h-8 text-primary/40" />
                    </div>
          ...
          <form onSubmit={handleSendAppreciation} className="space-y-3">
            ...
            <div>
              ...
              >
                ...
                {hashtags.map((h) => (
                  <option key={h.tag} value={h.tag}>{h.tag} - {h.desc}</option>
                ))}
              </select>
            </div>
          ...
          <div className="space-y-3 max-h-72 overflow-y-auto">
            {loading ? (
              ...
              transactions.map((tx) => {
                ...
                const iconColor = isDebit ? 'text-blue-600 bg-blue-50' : 'text-green-600 bg-green-50';
                return (
